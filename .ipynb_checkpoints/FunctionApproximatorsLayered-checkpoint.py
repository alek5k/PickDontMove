...
class PolicyEstimator():
    ...
    def __init__(self, env, learning_rate=0.01, common_hidden_size=[30,30], pickers_hidden=[10], agvs_hidden=[10], scope="policy_estimator"):
        ...
        with tf.variable_scope(scope):
            ...
            # COMMON
            input_layer = self.state
            self.common_layers = {}
            
            for i in range(len(common_hidden_size)):
                self.common_layers[i] = tf.contrib.layers.fully_connected(
                    inputs=input_layer, 
                    num_outputs=common_hidden_size[i+1], 
                    activation_fn=tf.nn.relu
                )
                
                input_layer = self.fully_connected
...
if __name__ == "__main__":
    ...
    policy_estimator = PolicyEstimator(env, learning_rate=0.001, hidden_size=30)
    value_estimator = ValueEstimator(env, learning_rate=0.001)